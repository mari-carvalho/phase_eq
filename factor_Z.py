...
def func_Z_liq(Z: float, B_final_liq: float, A_final_liq: float, delta_1: float, delta_2: float) -> float:
...
def calculate_Z_liq(func_Z_liq):
...
def func_Z_gas(Z: float, B_final_gas:float, A_final_gas: float, eec: str) -> float:
...
def calculate_Z_gas(func_Z_gas):
